...
* Get map of current cluster(on any `postgres` node): 
    * `gosu postgres repmgr cluster show` - tries to connect to all nodes on request ignore status of node in `$REPMGR_SCHEMA.$REPMGR_NODES_TABLE`
    * `gosu postgres psql $REPLICATION_DB -c "SELECT * FROM $REPMGR_SCHEMA.$REPMGR_NODES_TABLE"` - just select data from tables
