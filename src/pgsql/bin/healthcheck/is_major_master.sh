...
AM_I_MASTER=`PGPASSWORD=$REPLICATION_PASSWORD psql -h $CLUSTER_NODE_NETWORK_NAME -U $REPLICATION_USER $REPLICATION_DB  -tAc "SELECT * FROM $REPMGR_SCHEMA.$REPMGR_NODES_TABLE WHERE $REPMGR_NODE_NAME_COLUMN='$MY_NAME' AND (type='primary' OR type='master')" | wc -l`
...
if [ "$PARTNER_NODES" != "" ]; then
    ...
    DIFFERENT_UPSTREAMS=`PGPASSWORD=$REPLICATION_PASSWORD psql -h $CLUSTER_NODE_NETWORK_NAME -U $REPLICATION_USER $REPLICATION_DB  -tAc "SELECT upstream_node_id FROM $REPMGR_SCHEMA.$REPMGR_NODES_TABLE" | uniq | awk '{$1=$1};1' | grep -v -e '^[[:space:]]*$' | wc -l`
    ...
    read -d '' -ra NODES <<<`PGPASSWORD=$REPLICATION_PASSWORD psql -h $CLUSTER_NODE_NETWORK_NAME -U $REPLICATION_USER $REPLICATION_DB  -tAc "SELECT conninfo FROM $REPMGR_SCHEMA.$REPMGR_NODES_TABLE" | grep host | awk '{print $3}' | cut -d "=" -f2`
fi
...
for NODE in "${NODES[@]}"; do
    ...
    MASTER=`PGCONNECT_TIMEOUT=$CHECK_PGCONNECT_TIMEOUT PGPASSWORD=$REPLICATION_PASSWORD psql -h $NODE -U $REPLICATION_USER $REPLICATION_DB  -tAc "SELECT upstream_node_name FROM $REPMGR_SCHEMA.$REPMGR_SHOW_NODES_TABLE WHERE conninfo LIKE '%host=$NODE%' AND (upstream_node_name IS NOT NULL AND upstream_node_name <>'') AND active=true"`
    if [[ "$?" -ne "0" ]]; then
